# Task: <Outcome>

Use for multi-step work; skip for genuinely low-risk single-turn changes. Keep this file current before pause, handover, or completion. A session checklist is not a substitute for this durable file. A pre-approval drafting checkpoint is allowed; it authorizes no implementation.

## Goal and scope
- Mode: specification-only, implementation, or review-only; record the actual request, not an assumed default.
- Outcome / why:
- Requirement or issue / exact draft or approved PRD path and version:
- Scope approval evidence / approver / date / exclusions: pending for drafting; an explicit direct implementation request may define approved scope without a PRD.
- Execution authorization: not authorized for specification-only/review-only; otherwise exact user request and authorized write/operation scope:
- In scope / non-goals:
- Risk and required gates:

## Ownership and integration
- Exact task path (update on rename):
- Owner / team; optional session ID:
- Branch/worktree and base revision (or non-Git/unborn workspace state):
- Owned write paths:
- Dependencies / outstanding workers:
- Integration owner / shared files / merge order:

## Plan and acceptance
- Next steps within authorized mode (draft/review steps before implementation authorization):
- [ ] Observable outcome — required check:
- [ ] Error or regression behavior — required check:
- Canonical behavior/architecture targets; baseline and requirement IDs (or not applicable):

## Resume State
- Updated at / author:
- Completed / partial / not started:
- Exact next action or command and working directory:
- Current hypothesis / blockers / decision needed:
- Decisions and rejected approaches with reasons (link longer records):
- Current revision and uncommitted work location/fingerprint:
- Evidence still valid / invalidated and why:
- Relevant source, docs, and output paths (no secrets or transcript dump):
- Successor ownership transfer / outstanding coordination:

## Verification and closure
- Criterion / command or inspection / result / evidence location:
- Tested state and relevant environment for cross-session, high-risk, or release work:
- Combined-state checks and integration result:
- Current-doc reconciliation result / conflicts resolved:
- Optional durable learning updated, corrected, retired, or no-op:
- Failed / skipped / unverified checks and reasons:
- Recovery plan / operations reference if relevant:
- Implemented / integrated / deployed state and evidence:
- Status: active, blocked, or done for stated scope; pending deployment remains explicit.
